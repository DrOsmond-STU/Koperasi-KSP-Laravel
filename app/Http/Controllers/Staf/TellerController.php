...
class TellerController extends Controller
{
    ...
    public function preview(TellerSavingsTransactionRequest $request): View
    {
        ...
        $cashAccount = $this->resolveCashAccount($request);
        ...
        return view('staf.teller-preview', [
            ...
            'lines' => $this->savings->previewLines($account, $request->validated('type'), (float) $request->validated('amount'), $cashAccount),
        ]);
    }
    ...
    public function store(TellerSavingsTransactionRequest $request): RedirectResponse
    {
        ...
        $transaction = $request->validated('type') === 'setor'
            ? $this->savings->deposit($account, (float) $request->validated('amount'), $request->user()->id, $request->validated('description'), $idempotencyKey, $date, $cashAccount)
            : $this->savings->withdraw($account, (float) $request->validated('amount'), $request->user()->id, $request->validated('description'), $idempotencyKey, $date, $cashAccount);
    ...
    public function storeAccount(OpenSavingsAccountRequest $request): RedirectResponse
    {
        ...
        DB::transaction(function () use ($member, $productIds, $deposits, $request, $cashAccount, &$opened, &$skipped) {
            foreach ($productIds as $productId) {
                ...
                $account = $this->savings->openAccount($member, $product, $member->branch_id, $amount, $request->user()->id, $cashAccount);
    ...
    /**
     * Opsi akun kas yang bisa dipilih staf di field "Rekening Kas" pada
     * form Teller & Buka Rekening (permintaan staf 26 Agu 2026: field itu
     * sebelumnya cuma info banner read-only, sekarang bisa diedit — default
     * tetap SavingsService::cashAccount()). Query sama dengan
     * BranchCashSettingsController::index() (Postable + mengandung "kas" di
     * nama, supaya daftar tetap relevan walau Bagan Akun produksi bisa
     * ratusan baris) — akun default SELALU disertakan walau tidak lolos
     * filter itu, supaya field-nya tidak pernah gagal ter-preselect.
     */
    private function cashAccountOptions(): Collection
    {
        ...
        $default = $this->savings->cashAccount();

        if (! $accounts->contains('id', $default->id)) {
    ...
    /**
     * Staf boleh mengganti akun kas lawan transaksi dari default (lihat
     * SavingsService::cashAccount()) lewat field "Rekening Kas" — kosong/
     * tidak dikirim (mis. request lama yang belum tahu field ini) tetap
     * null, sehingga SavingsService jatuh ke default seperti sebelumnya.
     */
    private function resolveCashAccount(Request $request): ?ChartOfAccount
    {
        $id = $request->integer('cash_account_id') ?: null;

        return $id ? ChartOfAccount::query()->find($id) : null;
    }
}
