...
@section('content')
    ...
    <div class="form-card">
        <form method="POST" action="{{ route('admin.master.branches.store') }}">
            @csrf

            <div class="field-row">
                <div class="field">
                    <label for="code">Kode Cabang</label>
                    <input type="text" id="code" name="code" value="{{ old('code') }}" maxlength="30" required>
                    <div class="field-hint">Singkat dan unik, mis. <code>PST</code> atau <code>CAB-01</code>.</div>
                </div>
                <div class="field">
            ...
            <div class="field">
                ...
                <select id="parent_branch_id" name="parent_branch_id">
                    ...
                    @foreach ($parentOptions as $option)
                        <option value="{{ $option->id }}" @selected(old('parent_branch_id') == $option->id)>{{ $option->code }} — {{ $option->name }}</option>
                    @endforeach
                </select>
                <div class="field-hint">Kosongkan untuk kantor pusat atau cabang yang berdiri sendiri.</div>
            </div>

            <div class="field">
                <label for="operational_date">Tanggal Operasional</label>
                <input type="date" id="operational_date" name="operational_date" value="{{ old('operational_date', now()->toDateString()) }}">
            </div>

            <div class="field field-check">
                <label><input type="checkbox" name="is_active" value="1" @checked(old('is_active', true))> Cabang aktif</label>
            </div>
