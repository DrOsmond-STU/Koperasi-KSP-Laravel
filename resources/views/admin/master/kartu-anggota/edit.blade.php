...
@section('title', 'Ubah Template Kartu Anggota')
...
@section('content')
    <style>
        ...
        .current-bg { font-size: 11px; color: var(--muted); margin-top: 4px; }
    </style>

    <h2>Ubah Template — {{ $template->name }}</h2>
    ...
    <form method="POST" action="{{ route('admin.master.member-card-templates.update', $template) }}" enctype="multipart/form-data" id="designer-form">
        @csrf
        @method('PUT')
                ...
                <div class="panel">
                    ...
                    <div class="field-row">
                        <div class="field">
                            ...
                            <input type="number" step="0.01" name="width_mm" id="tpl-width" value="{{ old('width_mm', $template->width_mm) }}" required>
                        </div>
                        <div class="field">
                            <label>Tinggi (mm)</label>
                            <input type="number" step="0.01" name="height_mm" id="tpl-height" value="{{ old('height_mm', $template->height_mm) }}" required>
                        </div>
                        <div class="field">
                            <label>Warna Latar</label>
                            <input type="color" name="background_color" id="tpl-bg-color" value="{{ old('background_color', $template->background_color) }}">
                        </div>
                    </div>
                    <div class="field">
                        ...
                        @if ($template->background_image_path)
                            <p class="current-bg">Gambar latar saat ini terpasang — unggah file baru untuk mengganti.</p>
                        @endif
                    </div>
                </div>
    ...
    @php
        $existingFieldsForJs = $template->fields->map(fn ($f) => [
            'field_id' => $f->id,
            'field_key' => $f->field_key,
            'custom_text_value' => $f->custom_text_value,
            'position_x_mm' => (float) $f->position_x_mm,
            'position_y_mm' => (float) $f->position_y_mm,
            'width_mm' => $f->width_mm !== null ? (float) $f->width_mm : null,
            'height_mm' => $f->height_mm !== null ? (float) $f->height_mm : null,
    ...
    <script>
        (function () {
            ...
            var existingFields = @json($existingFieldsForJs);
            ...
            function addFieldRow(data) {
                ...
                var hiddenId = data.field_id ? '<input type="hidden" name="fields[' + index + '][field_id]" value="' + data.field_id + '">' : '';
                card.innerHTML = hiddenId +
            ...
            existingFields.forEach(function (field) { addFieldRow(field); });
