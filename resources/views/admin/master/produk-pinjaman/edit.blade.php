        ...
        <form method="POST" action="{{ route('admin.master.loan-products.update', $product) }}">
            ...
            <div class="field">
                ...
                <select name="tenor_unit" required>
                    <option value="hari" @selected(old('tenor_unit', $product->tenor_unit) === 'hari')>Hari (ditagih harian — mis. Pinjaman Anggota)</option>
                    <option value="bulan" @selected(old('tenor_unit', $product->tenor_unit) === 'bulan')>Bulan (potong gaji bulanan — mis. Piutang Karyawan)</option>
                </select>
                <p class="hint">Mengubah satuan HANYA berlaku untuk pengajuan pinjaman BARU sesudahnya — pinjaman yang sudah berjalan tetap terbaca dengan satuan saat ia dibuat (lihat Loan::usesDailyTenor()).</p>
            </div>
            <div class="grid-2">
                <div class="field"><label>Tenor Minimum</label><input type="number" name="min_tenor_days" value="{{ old('min_tenor_days', $product->min_tenor_days) }}" required></div>
                <div class="field"><label>Tenor Maksimum</label><input type="number" name="max_tenor_days" value="{{ old('max_tenor_days', $product->max_tenor_days) }}" required></div>
            </div>
            ...
            <div class="info-box">
                ...
                @if ($currentRate)
                    {{ number_format($currentRate->rate_percentage, 3, ',', '.') }}% {{ $product->usesDailyTenor() ? 'flat (seluruh tenor)' : '/tahun' }}
