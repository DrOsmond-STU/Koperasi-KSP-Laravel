...
@section('content')
    ...
    <div class="form-card">
        <form method="POST" action="{{ route('admin.master.loan-products.store') }}">
            ...
            <div class="grid-2">
                <div class="field"><label>Tenor Minimum (hari)</label><input type="number" name="min_tenor_days" value="{{ old('min_tenor_days', 7) }}" required></div>
                <div class="field"><label>Tenor Maksimum (hari)</label><input type="number" name="max_tenor_days" value="{{ old('max_tenor_days', 180) }}" required></div>
            </div>
            ...
            <div class="field">
                <label>Tarif Jasa Awal (%/tahun)</label>
                <input type="number" step="0.001" name="initial_rate_percentage" value="{{ old('initial_rate_percentage', 12) }}" required>
                <p class="hint">Disimpan sebagai bunga tahunan (APR); mesin perhitungan otomatis membagi 360 untuk mendapatkan tarif per hari (konvensi BI 30/360).</p>
            </div>
