...
@section('content')
    ...
    <h2>Jenis Anggota</h2>
    <p style="color: var(--muted); margin-top: -8px;">Kelola jenis keanggotaan koperasi (mis. Anggota Biasa, Calon Anggota, Anggota Kios).</p>
    ...
    <div class="toolbar">
        <span></span>
        <a href="{{ route('admin.master.member-types.create') }}" class="btn-primary">+ Tambah Jenis</a>
    </div>

    <table class="data-table">
        <thead>
            <tr>
                <th>Kode</th><th>Nama</th><th>Hak Suara</th><th>Simpanan Wajib</th><th>Anggota</th><th>Status</th><th>Aksi</th>
            </tr>
        </thead>
        <tbody>
            @forelse ($types as $type)
                <tr>
                    <td>{{ $type->code }}</td>
                    <td>{{ $type->name }}</td>
                    <td>{{ $type->has_voting_rights ? 'Ya' : 'Tidak' }}</td>
                    <td>{{ $type->requires_mandatory_savings ? 'Ya' : 'Tidak' }}</td>
                    <td>{{ $memberCounts[$type->id] ?? 0 }}</td>
                    <td><span class="pill {{ $type->is_active ? 'pill-aktif' : 'pill-other' }}">{{ $type->is_active ? 'Aktif' : 'Nonaktif' }}</span></td>
                    <td class="aksi-cell">
                        <a href="{{ route('admin.master.member-types.edit', $type) }}" class="btn-link">Ubah</a>
                        <form method="POST" action="{{ route('admin.master.member-types.destroy', $type) }}" onsubmit="return confirm('Hapus jenis anggota &quot;{{ $type->name }}&quot;?');">
                            @csrf
                            @method('DELETE')
            ...
            @empty
                <tr><td colspan="7">Belum ada jenis anggota — tambahkan yang pertama.</td></tr>
            @endforelse
        </tbody>
    </table>
@endsection
